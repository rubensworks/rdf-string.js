...
describe('TermUtil', () => {
  describe('#termToString', () => {
    ...
    it('should transform a named node', async() => {
      expect(TermUtil.termToString(FACTORY.namedNode('http://example.org'))).toBe('http://example.org');
    });

    it('should transform a blank node', async() => {
      expect(TermUtil.termToString(FACTORY.blankNode('b1'))).toBe('_:b1');
    });

    it('should transform a variable', async() => {
      expect(TermUtil.termToString(FACTORY.variable('v1'))).toBe('?v1');
    });

    it('should transform the default graph', async() => {
      expect(TermUtil.termToString(FACTORY.defaultGraph())).toBe('');
    });

    it('should transform a literal', async() => {
      expect(TermUtil.termToString(FACTORY.literal('abc'))).toBe('"abc"');
    });

    it('should transform a literal with a language', async() => {
      expect(TermUtil.termToString(FACTORY.literal('abc', 'en'))).toBe('"abc"@en');
    });
    ...
    it('should transform a literal with a datatype', async() => {
      expect(TermUtil.termToString(FACTORY.literal('abc', FACTORY.namedNode('http://ex')))).toBe('"abc"^^http://ex');
    });
  ...
  describe('#getLiteralValue', () => {
    it('should error on invalid literals', async() => {
      expect(() => TermUtil.getLiteralValue('"abc')).toThrow('"abc is not a literal');
    });
  });

  describe('#getLiteralType', () => {
    it('should error on invalid literal types', async() => {
      expect(() => TermUtil.getLiteralType('"abc"h')).toThrow('"abc"h is not a literal');
    });
  });

  describe('#getLiteralLanguage', () => {
    it('should error on invalid languages', async() => {
      expect(() => TermUtil.getLiteralLanguage('"abc"@')).toThrow('"abc"@ is not a literal');
    });
  });

  describe('#getLiteralDirection', () => {
    it('should error on invalid directions', async() => {
      expect(() => TermUtil.getLiteralDirection('"abc"@en--bla'))
        .toThrow('"abc"@en--bla is not a literal with a valid direction');
    });
  });
  ...
  describe('#stringToTerm', () => {
    ...
    describe('with a custom data factory', () => {
      ...
      it('should transform nested quads with literal', async() => {
        expect(TermUtil.stringToTerm('<<<<ex:s ex:p ex:o>> ex:p <<ex:s ex:p "s">>>>', FACTORY))
          .toEqual(FACTORY.quad(
            FACTORY.quad(
              FACTORY.namedNode('ex:s'),
              FACTORY.namedNode('ex:p'),
      ...
      it('should transform nested quads with many spaces and angle bracket named nodes', async() => {
        expect(TermUtil.stringToTerm('<< <<ex:s ex:p ex:o  >>   <p> << <s> ex:p ex:o>> >>', FACTORY))
          .toEqual(FACTORY.quad(
            FACTORY.quad(
              FACTORY.namedNode('ex:s'),
              FACTORY.namedNode('ex:p'),
      ...
      it('should error on a quad with extra closing tag', async() => {
        expect(() => TermUtil.stringToTerm('<<>>>', FACTORY))
